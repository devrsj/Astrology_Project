...
@php
$service_pages=App\Models\SubCategory::where('category_id',$service_page->id)->OrderBy('weight','asc')->where('status',1)->simplePaginate(17);
@endphp
@endif
...
@section('content')
 ...
 @if(isset($service))
 <section class="as_breadcrum_wrapper" @if(isset($service)) style="background-image: url({{ url('/') }}/storage/posts/ @if(isset($service)) {{ $service->banner }}) @endif
        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-12 text-center">
                    <h1>
                        @if(isset($service)) {{ $service->name }} @endif  @if(isset($service_page)) {{ $service_page->name }} @endif
                        </h1>

                    <ul class="breadcrumb">
                        <li><a href="{{ url('/') }}">Home</a></li>
                      
                        <li> @if(isset($service)) {{ $service->name }} @endif  @if(isset($service_page)) {{ $service_page->name }} @endif</li>
                    </ul>
                </div>
            </div>
        </div>
...
<div class="row">
    <div class="col-sm-12 col-md-12 col-lg-12">
        <h4> @if(isset($service)) {{ $service->name }} @endif  @if(isset($service_page)) {{ $service_page->name }} @endif</h4>
        <hr>
        <p> @if(isset($service)) {!! $service->description !!} @endif @if(isset($service_page)) {!! $service_page->description !!} @endif</p>
    </div>
</div>
...
<a href="{{ route('page.slug',$subundercat->slug) }}" class="as_link">Read more</a>
  <a href="{{ route('addto.cart',$subundercat->id) }}" class="as_link">Add to Cart</a>
                ...
                @endif
                  ...
                  @foreach($service_pages as $subundercat)
                <div class="col-lg-4 col-md-6 col-sm-6 ">
                    <div class="as_service_box text-center">
                        <figure >
 ...
 <a href="{{ route('page.slug',$subundercat->slug) }}" class="as_link" style="width:100%">Read more</a>
            ...
            @endif
              ...
              @if(isset($service_page))  
            @if($service_pages->count() > 0)
            {{ $service_pages->links('pagination::simple-default') }}
            @endif
            @endif
